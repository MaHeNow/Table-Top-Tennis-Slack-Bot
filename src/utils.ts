...
export async function getUsersWithScores() {
    ...
    const userToScores: {[userID: string]: number} = {};

    for (const match of matches) {

        const player1Won = match.player1Score > match.player2Score;
        const winner = player1Won ? match.player1ID : match.player2ID;
        const looser = player1Won ? match.player2ID : match.player1ID;

        if (!(winner in userToScores)) {
            userToScores[winner] = 3
        } else {
            userToScores[winner] += 3;
        }

        if (!(looser in userToScores)) {
            userToScores[looser] = 0;
        }
    }
